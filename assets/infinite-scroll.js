...
class InfiniteScroll {
  ...
  createLoadingIndicator() {
    ...
    this.elements.productGrid.insertAdjacentHTML('afterend', loadingHTML);
    this.elements.productGrid.insertAdjacentHTML('afterend', loadMoreHTML);
  ...
  setupLoadMoreButton() {
    ...
    const button = this.elements.loadMoreButton.querySelector('button');
  ...
  processResponse(html, page) {
    ...
    const newProducts = newProductGrid ? newProductGrid.querySelectorAll('.grid__item') : [];
    ...
    newProducts.forEach(product => {
      this.elements.productGrid.appendChild(product.cloneNode(true));
    });
    ...
    if (!pagination || !pagination.querySelector('.pagination__item--prev')) {
      this.hasMoreProducts = false;
      this.hideLoadMoreButton();
    }
    ...
    document.dispatchEvent(new CustomEvent('infiniteScroll:productsLoaded', {
      detail: { 
        page: page, 
  ...
  detectInitialState() {
    ...
    if (this.elements.pagination) {
      const nextPageLink = this.elements.pagination.querySelector('.pagination__item--prev');
      this.hasMoreProducts = !!nextPageLink;
    }
  ...
  showError() {
    ...
    if (this.elements.loadingIndicator) {
      this.elements.loadingIndicator.insertAdjacentHTML('afterend', errorHTML);
    }
  }
